...
.SYNOPSIS
LanguageChange - Installs a scheduled task on a Windows system to change all language and region settings to desired values.
Uses: PSAppDeployToolkit - This script performs the installation or uninstallation of an application(s).

.DESCRIPTION
- The script either performs an "Install", "Uninstall", or "Repair" deployment type.
- Install will create the Task in Task Scheduler and move the script to a trusted location
- Uninstall will remove just the script, registry keys used in the script and the Task.
...
#Reccommended install command for this application.
...
$adtSession = @{
    # App variables.
    AppVendor = 'Harris Bowman'
    AppName = 'System Language Change'
    AppVersion = '5.7'
    AppArch = 'x64'
    AppLang = 'EN'
    AppRevision = '01'
    ...
    AppScriptVersion = '5.7.0'
    AppScriptDate = '2025-10-14'
    AppScriptAuthor = 'Harris Bowman'
    RequireAdmin = $true

    # Install Titles (Only set here to override defaults set by the toolkit).
    InstallName = 'System Language Change'
    InstallTitle = ''
...
function Install-ADTDeployment
{
    ...
    if (Test-Path -Path "C:\Program Files\LanguageChange\") {
        ...
        New-ADTFolder -LiteralPath "C:\Program Files\LanguageChange\"
        Copy-ADTFile -Path "$($adtSession.DirFiles)\Invoke-ChangeDefaultLanguage.ps1" -Destination "C:\Program Files\LanguageChange\Invoke-ChangeDefaultLanguage.ps1"
    }
    ...
    $actions = New-ScheduledTaskAction -Execute "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe" -Argument '-WindowStyle Hidden -NonInteractive -ExecutionPolicy Bypass -file "C:\Program Files\LanguageChange\Invoke-ChangeDefaultLanguage.ps1"'
    $trigger = New-ScheduledTaskTrigger -AtLogon
    $principal = New-ScheduledTaskPrincipal -GroupId "BUILTIN\Administrators" -RunLevel Highest
    $settings = New-ScheduledTaskSettingsSet -RunOnlyIfNetworkAvailable -WakeToRun -ExecutionTimeLimit (New-TimeSpan -Hours 1) -DontStopIfGoingOnBatteries -MultipleInstances IgnoreNew -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 20)

    $task = New-ScheduledTask -Action $actions -Principal $principal -Trigger $trigger -Settings $settings -Description "At logon, will change all system language settings to use the languages and region specified in the Invoke-ChangeDefualtLanguage script. Does not run the entire script if settings are already set"

    Register-ScheduledTask 'Change System Language' -InputObject $task
    ...
    Start-ScheduledTask -TaskName "Change System Language"
...
function Repair-ADTDeployment
{
    ...
    Stop-ScheduledTask -TaskName "Change System Language"
